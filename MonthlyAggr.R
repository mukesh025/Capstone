...
colnames(FY12_Data)[49]<-"HARDWARE_FAMILY"
...
FY12_agg_data<-ddply(FY12_Data,c("HARDWARE_FAMILY","FISCAL_MONTH"),summarize,
...
FY13_agg_data<-ddply(FY13_Data,c("HARDWARE_FAMILY","FISCAL_MONTH"),summarize,
                     mean = round(mean(Q8OverallSatisfaction),2))
...
FY14_agg_data<-ddply(FY14_Data,c("HARDWARE_FAMILY","FISCAL_MONTH"),summarize,
                     mean = round(mean(Q8OverallSatisfaction),2))
...
FY15_agg_data<-ddply(FY15_Data,c("HARDWARE_FAMILY","FISCAL_MONTH"),summarize,
                     mean = round(mean(Q8OverallSatisfaction),2))
...
FY16_agg_data<-ddply(FY16_Data,c("HARDWARE_FAMILY","FISCAL_MONTH"),summarize,
...
FY16Q3_agg_data<-ddply(FY16Q3_Data,c("HARDWARE_FAMILY","FISCAL_MONTH"),summarize,
                     mean = round(mean(Q8OverallSatisfaction)/2,2))
...
df1 <- tbl_df(AggData)
AggData<-arrange_(df1, c('HARDWARE_FAMILY', 'FISCAL_MONTH'))

#change column name for 'mean' column as it stores average of overall satisfaction score
colnames(AggData)[3]<-"AVG_OVERALL_SATISFACTION"

#Save Aggregate data in csv file

write.csv(AggData, file="MONTHYLY_TSAT.csv")
